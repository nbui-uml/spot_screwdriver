...
class SpotDetection:
    def __init__(self, net: str = None) -> None:
        """
        Initializes detection model based on net.

        Parameters
        -----
        net: str
            Path to YOLOv5 ONNX file.
        """
...
def detect_valid_objects(robot: bosdyn.client.Robot, net: cv2.dnn.Net) -> bool:
    """
    ...
    robot: Robot
        Robot object.
    net: cv2.dnn.Net
        Net object for the YOLOv5 model
    ...
    for camera in cameras:
        ...
        rotated_img = ndimage.rotate(img, ROTATION_ANGLE[visual_image.shot.frame_name_image_sensor])

        #send image through detection model
        formatted_img = format_yolov5(rotated_img)
        out = detect(formatted_img, net)
        class_ids, confidences, boxes = wrap_detection(formatted_img, out[0])
        ...
        mapped_img = reverse_rotation_map(img, rotated_img, ROTATION_ANGLE[visual_image.shot.frame_name_image_sensor])
        ...
        valid_points = get_valid_points(visual_image, depth_image, transformed_points)
        ...
        if len(valid_points) == 0:
            ...
            cx = valid_points[0][0]
            cy = valid_points[0][1]
            grasp_from_image(visual_image, cx, cy)
            return True
    return False
